...
type ApiResponse = {
  data: ListFacilitiesQuery;
};

const useFacility = (
  variables: ListFacilitiesQueryVariables = {}
): {
    ...
    const f = async () => {
      ...
      try {
        const result = (await API.graphql(
          graphqlOperation(listFacilities, variables.filter)
        )) as ApiResponse;
        setList(result.data.listFacilities.items);
      } catch {
        setError({
          isError: true,
